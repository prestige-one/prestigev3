<template>
 <div>
    <!-- Standard Blog Post -->
    <div v-if="item.type === 'standard'" class="tp-blog-masonry-item">
      <div class="tp-blog-masonry-item-top d-flex justify-content-between align-items-center mb-30">
        <div class="tp-blog-masonry-item-user d-flex align-items-center">
          <div class="tp-blog-masonry-item-user-thumb">
            <img :src="item.author.image" :alt="item.author.name">
          </div>
          <div class="tp-blog-masonry-item-user-content">
            <span>{{ item.author.name }}</span>
            <p>{{ item.author.role }}</p>
          </div>
        </div>
        <div class="tp-blog-masonry-item-time">
          <span>
            <svg-clock-one/> 
            {{ item.date }}
          </span>
        </div>
      </div>
      <div class="tp-blog-masonry-thumb mb-30">
        <nuxt-link href="/blog-details">
          <img :src="item.image" :alt="item.title">
        </nuxt-link>
      </div>
      <div class="tp-blog-masonry-content">
        <div class="tp-blog-masonry-tag mb-15">
          <span>
            <svg-clock-one/>
            {{ item.category }}
          </span>
        </div>
        <h4 class="tp-blog-masonry-title">
          <nuxt-link class="tp-line-white" href="/blog-details">
            
            <span v-html="sanitizeHtml(item.title)"/>
          </nuxt-link>
        </h4>
        <div class="tp-blog-masonry-btn">
          <nuxt-link href="/blog-details">
            Read More 
            <span>
              <svg-arrow-up-eight/>
              <svg-arrow-up-eight/>
            </span>
          </nuxt-link>
        </div>
      </div>
    </div>

    <!-- Text-only Blog Post -->
    <div v-else-if="item.type === 'text-only'" class="tp-blog-masonry-item-2">
      <div class="tp-blog-masonry-item-top d-flex justify-content-between align-items-center mb-30">
        <div class="tp-blog-masonry-item-user d-flex align-items-center">
          <div class="tp-blog-masonry-item-user-thumb">
            <img :src="item.author.image" :alt="item.author.name">
          </div>
          <div class="tp-blog-masonry-item-user-content">
            <span>{{ item.author.name }}</span>
            <p>{{ item.author.role }}</p>
          </div>
        </div>
        <div class="tp-blog-masonry-item-time">
          <span>
            <svg-clock-one/> 
            {{ item.date }}
          </span>
        </div>
      </div>
      <div class="tp-blog-masonry-content">
        <div class="tp-blog-masonry-tag mb-15">
          <span>
            <svg-tag-one/>
            {{ item.category }}
          </span>
        </div>
        <h4 class="tp-blog-masonry-title">
          <nuxt-link class="tp-line-white" href="/blog-details">
            
            <span v-html="sanitizeHtml(item.title)"/>
          </nuxt-link>
        </h4>
        <div class="tp-blog-masonry-btn">
          <a href="/blog-details">
            Read More 
            <span>
              <svg-arrow-up-eight/>
              <svg-arrow-up-eight/>
            </span>
          </a>
        </div>
      </div>
    </div>

    <!-- Video Blog Post -->
    <div v-else-if="item.type === 'video'" class="tp-blog-masonry-item">
      <div class="tp-blog-masonry-item-top d-flex justify-content-between align-items-center mb-30">
        <div class="tp-blog-masonry-item-user d-flex align-items-center">
          <div class="tp-blog-masonry-item-user-thumb">
            <img :src="item.author.image" :alt="item.author.name">
          </div>
          <div class="tp-blog-masonry-item-user-content">
            <span>{{ item.author.name }}</span>
            <p>{{ item.author.role }}</p>
          </div>
        </div>
        <div class="tp-blog-masonry-item-time">
          <span>
            <svg-clock-one/>
            {{ item.date }}
          </span>
        </div>
      </div>
      <div class="tp-blog-masonry-thumb p-relative mb-30">
        <div class="tp-blog-masonry-thumb-video">
          <modal-video cls="popup-video" :url="`https://www.youtube.com/watch?v=${item.videoId}`" :show-icon="false">
              <span>
                <svg-video-play-three fill="#0E0F11"/>
              </span>
          </modal-video>
        </div>
        <a href="/blog-details">
          <img :src="item.image" :alt="item.title">
        </a>
      </div>
      <div class="tp-blog-masonry-content">
        <div class="tp-blog-masonry-tag mb-15">
          <span>
            <svg-tag-one/>
            {{ item.category }}
          </span>
        </div>
        <h4 class="tp-blog-masonry-title">
          <nuxt-link class="tp-line-white" href="/blog-details">
            
            <span v-html="sanitizeHtml(item.title)"/>
          </nuxt-link>
        </h4>
        <div class="tp-blog-masonry-btn">
          <nuxt-link href="/blog-details">
            Read More 
            <span>
              <svg-arrow-up-eight/>
              <svg-arrow-up-eight/>
            </span>
          </nuxt-link>
        </div>
      </div>
    </div>

    <!-- Quote Blog Post -->
    <div v-else-if="item.type === 'quote'" class="tp-blog-masonry-item-3">
      <div class="tp-blog-masonry-item-icon">
        <span>
          <svg xmlns="http://www.w3.org/2000/svg" width="26" height="16" viewBox="0 0 26 16" fill="none">
            <path d="M2.21529 16L1.80505 15.0881C2.57083 14.7565 3.30926 14.3696 4.02034 13.9275C4.73142 13.4853 5.36045 13.0155 5.90743 12.5181C6.50912 11.9655 6.97405 11.4404 7.30224 10.943L6.48177 10.1969H5.66129C4.51262 10.1969 3.5007 9.97582 2.62553 9.53368C1.80505 9.09154 1.14867 8.48359 0.656382 7.70984C0.218794 6.9361 0 6.05181 0 5.05699C0 4.06218 0.246143 3.1779 0.738429 2.40415C1.28541 1.6304 1.99649 1.05009 2.87167 0.663214C3.74684 0.221071 4.73142 0 5.82539 0C6.91935 0 7.90393 0.221071 8.7791 0.663214C9.65428 1.10536 10.338 1.7133 10.8303 2.48705C11.3226 3.2608 11.5687 4.14508 11.5687 5.1399C11.5687 6.35579 11.3226 7.51641 10.8303 8.62176C10.3927 9.67185 9.76367 10.6667 8.9432 11.6062C8.12272 12.5458 7.13815 13.4024 5.98948 14.1762C4.84081 14.8946 3.58275 15.5026 2.21529 16ZM16.6466 16L16.2363 15.0881C17.0021 14.7565 17.7405 14.3696 18.4516 13.9275C19.1627 13.4853 19.7917 13.0155 20.3387 12.5181C20.9404 11.9655 21.4053 11.4404 21.7335 10.943L20.913 10.1969H20.0926C18.9439 10.1969 17.932 9.97582 17.0568 9.53368C16.2363 9.09154 15.5799 8.48359 15.0877 7.70984C14.6501 6.9361 14.4313 6.05181 14.4313 5.05699C14.4313 4.06218 14.6774 3.1779 15.1697 2.40415C15.7167 1.6304 16.4278 1.05009 17.3029 0.663214C18.1781 0.221071 19.1627 0 20.2567 0C21.3506 0 22.3352 0.221071 23.2104 0.663214C24.0856 1.10536 24.7693 1.7133 25.2616 2.48705C25.7539 3.2608 26 4.14508 26 5.1399C26 6.35579 25.7539 7.51641 25.2616 8.62176C24.824 9.67185 24.195 10.6667 23.3745 11.6062C22.554 12.5458 21.5694 13.4024 20.4208 14.1762C19.2721 14.8946 18.014 15.5026 16.6466 16Z" fill="#ff5722" />
          </svg>
        </span>
      </div>
      <div class="tp-blog-masonry-tag mb-15">
        <span>
          <svg-clock-one/>
          {{ item.category }}
        </span>
      </div>
      <div class="tp-blog-masonry-item-text">
        
        <span v-html="sanitizeHtml(item.content)"/>
        <p>{{ item.authorName }}</p>
      </div>
    </div>

    <!-- Link Blog Post -->
    <div v-else-if="item.type === 'link'" class="tp-blog-masonry-item-3">
      <div class="tp-blog-masonry-item-icon">
        <span>
          <svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" viewBox="0 0 22 22" fill="none">
            <path d="M9.19683 11.906C9.58593 12.4262 10.0823 12.8566 10.6524 13.168C11.2225 13.4795 11.8528 13.6647 12.5008 13.7111C13.1487 13.7575 13.799 13.664 14.4077 13.437C15.0163 13.21 15.569 12.8547 16.0282 12.3953L18.7463 9.67721C19.5715 8.82282 20.0281 7.67851 20.0178 6.49073C20.0074 5.30295 19.531 4.16675 18.6911 3.32683C17.8512 2.48692 16.715 2.01049 15.5272 2.00017C14.3394 1.98985 13.1951 2.44646 12.3407 3.27165L10.7824 4.82094M12.8211 10.094C12.432 9.57382 11.9356 9.14341 11.3655 8.83196C10.7955 8.52051 10.1651 8.3353 9.51716 8.2889C8.86922 8.24249 8.21889 8.33598 7.61026 8.56301C7.00164 8.79005 6.44896 9.14532 5.98971 9.60474L3.27165 12.3228C2.44646 13.1772 1.98985 14.3215 2.00017 15.5093C2.01049 16.697 2.48692 17.8332 3.32683 18.6732C4.16675 19.5131 5.30295 19.9895 6.49073 19.9998C7.67851 20.0101 8.82282 19.5535 9.6772 18.7283L11.2265 17.1791" stroke="#ff5722" stroke-width="3" stroke-linecap="round" stroke-linejoin="round" />
          </svg>
        </span>
      </div>
      <div class="tp-blog-masonry-tag mb-15">
        <span>
          <svg-clock-one/>
          {{ item.category }}
        </span>
      </div>
      <div class="tp-blog-masonry-item-text">
        
        <span v-html="sanitizeHtml(item.content)"/>
        <p>{{ item.authorName }}</p>
      </div>
    </div>
  </div>
</template>

<script setup lang="ts">
// eslint-disable-next-line @typescript-eslint/no-explicit-any
defineProps<{item:any}>()
</script>
